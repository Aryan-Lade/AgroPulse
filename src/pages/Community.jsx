...
import {
  ...
  HiOutlineHeart, HiOutlineChatBubbleLeft, HiOutlineEye,
  HiOutlinePencilSquare, HiOutlineCheckBadge,
...
function Avatar({ initials, size = 'md' }) {
  ...
  return (
    <div className={classNames('rounded-xl bg-gradient-to-br from-primary-600 to-emerald-700 flex items-center justify-center font-bold text-white shrink-0', sz)}>
      {initials}
    </div>
  )
}
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {post.solved && <HiOutlineCheckBadge className="text-primary-400 text-base" title="Solved" />}
            <span className={classNames('inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border', TAG_STYLES[post.tag] ?? 'bg-surface-2 text-ink-2 border-line')}>
              {post.tag}
            </span>
          </div>
          <p className="text-xs text-ink-3">{post.author.role} · {post.timeAgo}</p>
        </div>
        <Badge status="neutral">{post.category}</Badge>
      </div>

      {/* Content */}
      <h3 className="font-display font-semibold text-ink text-sm mb-2 leading-snug">{post.title}</h3>
      <p className="text-xs text-ink-2 leading-relaxed line-clamp-3">{post.body}</p>
  ...
  return (
    <PageWrapper>
      <PageHeader
        ...
        action={
          <Button icon={HiOutlinePencilSquare} size="sm"
            onClick={() => toast.info('Create Post', 'Post creation coming soon.')}>
            New Post
          </Button>
        }
      />
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex flex-col gap-5">
            ...
            {visible.length === 0 && (
              <motion.div variants={fadeInUp} className="glass-card py-16 flex flex-col items-center gap-3">
                <HiOutlineUserGroup className="text-4xl text-ink-3" />
                <p className="text-ink-3 text-sm">No posts match your search.</p>
              </motion.div>
            )}
          </motion.div>
        </div>
        ...
        <div className="xl:w-72 shrink-0">
          <Card hover={false} className="sticky top-24">
            <h3 className="font-display font-semibold text-ink mb-4">Top Contributors</h3>
              ...
              {contributors.map((c, i) => (
                <div key={c.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-ink-3">{c.role} · {c.posts} posts</p>
                  </div>
            ...
            <div className="mt-6 pt-4 border-t border-line">
              <h3 className="font-semibold text-ink text-sm mb-3">Community Stats</h3>
